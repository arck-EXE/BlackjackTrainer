...
public class GameManager : MonoBehaviour
{
    ...
    private PlayerAction ConvertGameActionToPlayerAction(GameAction action)
    {
        return action switch
        {
            GameAction.Hit => PlayerAction.Hit,
            GameAction.Stand => PlayerAction.Stand,
            GameAction.Split => PlayerAction.Split,
            GameAction.DoubleDown => PlayerAction.Double,
            _ => PlayerAction.None,
        };
    }
    ...
    private async void HandleGameActionAsync(object sender, GameVisualManager.ActionBtnClickedEventArgs e)
    {
        ...
        // Prepare values needed for feedback
        var playerCards = _handsManager.CurrPlayerHand.GetCards(); // Replace with actual method to get List<CardSO>
        CardSO dealerUpcard = _handsManager.DealerHand.GetCardSO(0);

        PlayerAction playerAction = ConvertGameActionToPlayerAction(gameAction); // Add this method below

        // Show feedback
        string feedback = StrategyFeedback.GetFeedback(playerCards, dealerUpcard, playerAction);

        if (_feedbackPopup != null)
        {
            _feedbackPopup.ShowPopup(feedback);
        }
        else
        {
            Debug.LogError("FeedbackPopup is not assigned in the GameManager!");
        }
